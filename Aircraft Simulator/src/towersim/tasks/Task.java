...
public class Task implements Encodable {
    ...
    public String encode() {
        if (type.equals(TaskType.LOAD)) {
            return "LOAD@" + getLoadPercent();
        }
        return getType().name();
    }
    ...
    public boolean equals(Object obj) {
        if (getClass().getSimpleName().equals(obj.getClass().getSimpleName())) {
            if (getType().name().equals(((Task) obj).getType().name())) {
                return getLoadPercent() == ((Task) obj).getLoadPercent();
            }
        }
        return false;
    }
    ...
    @Override
    public int hashCode() {
        //The number 20 is used to create a unique hash code for every task.
        // The number 20 was arbitrarily chosen and has no inherent affect
        // if the integer is changed (except 0 and -ve numbers).
        return (this.getLoadPercent() + this.getType().name().length()) * 20;
    }
