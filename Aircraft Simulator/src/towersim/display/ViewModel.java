...
public class ViewModel {
     ...
     *                                #createControlTower(Reader, Reader, Reader, Reader)}
    ...
    public EventHandler<ActionEvent> getFindSuitableGateHandler() {
        return actionEvent -> {
            try {
                if (getSelectedAircraft().getValue() == null) {
                    return;
                }
                if (getSelectedAircraft().getValue().getTaskList().getCurrentTask().getType()
                        != TaskType.LAND) {
                    return;
                }
                suitableGateText.set(getControlTower()
                        .findUnoccupiedGate(getSelectedAircraft().getValue()).toString());
     ...
     * @param queuesWriter             writer to which the takeoff/landing queues and loading
     *                                 map will be written
    ...
    public void saveAs(Writer tickWriter, Writer aircraftWriter, Writer queuesWriter,
        ...
        // Code for tick save
        BufferedWriter bufferedTick = new BufferedWriter(tickWriter);
        bufferedTick.write(String.valueOf(getControlTower().getTicksElapsed()));
        bufferedTick.close();

        // Code for aircraft save
        BufferedWriter bufferedAircraft = new BufferedWriter(aircraftWriter);
        bufferedAircraft.write(String.valueOf(getControlTower().getAircraft().size()));
        if (getControlTower().getAircraft().size() != 0) {
            bufferedAircraft.write(System.lineSeparator());
        }
        if (getControlTower().getAircraft().size() != 0) {
            for (Aircraft aircraft : getControlTower().getAircraft()) {
                bufferedAircraft.write(aircraft.encode());
                bufferedAircraft.write(System.lineSeparator());
            }
        }
        bufferedAircraft.close();

        // Code for queues save
        BufferedWriter bufferedQueues = new BufferedWriter(queuesWriter);
        bufferedQueues.write(getControlTower().getTakeoffQueue().encode());
        if (getControlTower().getTakeoffQueue().getAircraftInOrder().size() != 0) {
            bufferedQueues.write(System.lineSeparator());
        }
        bufferedQueues.write(getControlTower().getLandingQueue().encode());
        if (getControlTower().getLandingQueue().getAircraftInOrder().size() != 0) {
            bufferedQueues.write(System.lineSeparator());
        }
        bufferedQueues.write("LoadingAircraft:" + getControlTower().getLoadingAircraft().size());
        if (getControlTower().getLoadingAircraft().size() != 0) {
            bufferedQueues.write(System.lineSeparator());
        }
        StringJoiner joiner = new StringJoiner(",");
        for (Map.Entry<Aircraft, Integer> entry :
                getControlTower().getLoadingAircraft().entrySet()) {
            String callsign = entry.getKey().getCallsign();
            int loadTime = entry.getValue();
            joiner.add(String.format("%s:%d", callsign, loadTime));
        }
        bufferedQueues.write(joiner.toString());
        bufferedQueues.close();

        // Code for terminals save
        BufferedWriter bufferedTerminals = new BufferedWriter(terminalsWithGatesWriter);
        bufferedTerminals.write(String.valueOf(getControlTower().getTerminals().size()));
        if (getControlTower().getTerminals().size() != 0) {
            bufferedTerminals.write(System.lineSeparator());
        }
        for (Terminal terminal : getControlTower().getTerminals()) {
            bufferedTerminals.write(terminal.encode());
            if (terminal.getGates().size() != 0) {
                bufferedTerminals.write(System.lineSeparator());
            }
        }
        bufferedTerminals.close();
    }
