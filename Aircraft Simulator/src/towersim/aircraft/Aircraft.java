...
public abstract class Aircraft implements OccupancyLevel, Tickable, EmergencyState, Encodable {
     ...
     * After calling unload(), OccupancyLevel.calculateOccupancyLevel() should return 0
    ...
    public boolean equals(Object obj) {
        if (obj instanceof Aircraft) {
            if (this.callsign.equals(((Aircraft) obj).callsign)) {
                return this.characteristics.equals(((Aircraft) obj).characteristics);
            }
        }
        return false;
    }
     ...
     * In this method the number 20 is multiplied to create a random number based on the aircrafts
     * properties.
     *
    ...
    @Override
    public int hashCode() {
        return (this.callsign.length() + (int) this.fuelAmount + this.tasks.toString().length())
                * 20;
    }
    ...
    public String encode() {
        ...
        joiner.add(getCallsign());
        joiner.add(getCharacteristics().toString());
        joiner.add(getTaskList().encode());
        joiner.add(String.format("%.2f", getFuelAmount()));
