...
public class Gate implements Encodable {
    ...
    public boolean equals(Object obj) {
        if (this.getClass().getSimpleName().equals(obj.getClass().getSimpleName())) {
            return getGateNumber() == ((Gate) obj).getGateNumber();
        }
        return false;
    }
     ...
     * <p>
     * In this case the number 20 is used to create a unique hashcode for every gate.
     *
     * @return hash code of this gate
     */
    @Override
    public int hashCode() {
        //The number 20 is used to create a unique hash code for every gate. The number 20 was
        // arbitrarily chosen and has no inherent affect if the integer is changed
        // (except 0 and -ve numbers).
        return this.getGateNumber() * 20;
    }
     ...
     * callsign is the callsign of the aircraft parked at this gate, or empty if the gate
     * is unoccupied
     * <p>
     * For example:
     * 12:ABC102
    ...
    public String encode() {
        String gateOccupancy = "empty";
        if (isOccupied()) {
            gateOccupancy = getAircraftAtGate().getCallsign();
        }
        return getGateNumber() + ":" + gateOccupancy;
    }
