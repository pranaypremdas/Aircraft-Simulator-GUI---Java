...
public abstract class Terminal implements EmergencyState, OccupancyLevel, Encodable {
    ...
    public boolean equals(Object obj) {
        if (this.getClass().getSimpleName().equals(obj.getClass().getSimpleName())) {
            return (getTerminalNumber() == ((Terminal) obj).getTerminalNumber());
        }
        return false;
    }
     ...
     * In this case the number 20 is used to create a unique hashcode for every terminal.
     *
     * @return hash code of this terminal
     */
    public int hashCode() {
        //The number 20 is used to create a unique hash code for every terminal. The number 20 was
        //  arbitrarily chosen and has no affect if the integer is changed
        // (except 0 and -ve numbers).
        return (this.getTerminalNumber() +  this.getGates().size()) * 20;
    }
     ...
     * 1 and N inclusive, where N is the number of gates, in the same order as returned by
     * getGates()
     * <p>
     * For example:
    ...
    public String encode() {
        ...
        joiner.add(String.valueOf(getTerminalNumber()));
        joiner.add(String.valueOf(hasEmergency()));
        joiner.add(String.valueOf(getGates().size()));

        StringJoiner joiner1 = new StringJoiner(System.lineSeparator());
        for (Gate gate : getGates()) {
            joiner1.add(gate.encode());
        }
        return joiner + System.lineSeparator() +  joiner1;
    }
}
